...
AUTHORIZED_USERS = [6897739611]  # Replace with actual admin user IDs
...
7. /terminal - Access Terminal Directly From Bot.
8. /upload - Upload Files Directly To The Vps Or Hosting.
9. /ls - List Names Of All The Files In The Vps Or Hosting
...
def handle_file(message):
    ...
    if UPLOAD_REQUESTS.get(user_id):
        try:
            # Get file info
            ...
            bot.reply_to(message, f"File '{file_name}' has been successfully uploaded to the VPS.")
        except Exception as e:
            bot.reply_to(message, f"An error occurred while uploading the file:\n{e}")
        finally:
            # Remove the user from upload tracking
            UPLOAD_REQUESTS.pop(user_id, None)
    else:
        bot.reply_to(message, "Please use /upload first to start the upload process.")
...
def ls(message):
    ...
    try:
        ...
        if files:
            file_list = "\n".join(files)
            bot.reply_to(message, f"Files in the current directory:\n{file_list}")
        else:
            bot.reply_to(message, "The directory is empty.")
    except Exception as e:
        bot.reply_to(message, f"An error occurred while listing files:\n{e}")
